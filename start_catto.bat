...
set "M_LTA=0" & findstr /r "^LTA_ACCOUNT_KEY=[^ ]" .env >nul 2>&1 && (set "LOADED_KEYS=!LOADED_KEYS! LTA") || (set "M_LTA=1" & set "MISSING_KEYS=1")
set "M_SKY=0" & findstr /r "^OPENSKY_CLIENT_ID=[^ ]" .env >nul 2>&1 && (set "LOADED_KEYS=!LOADED_KEYS! OpenSky") || (set "M_SKY=1" & set "MISSING_KEYS=1")
set "M_AIS=0" & findstr /r "^AIS_API_KEY=[^ ]" .env >nul 2>&1 && (set "LOADED_KEYS=!LOADED_KEYS! AIS") || (set "M_AIS=1" & set "MISSING_KEYS=1")
set "M_OCX=0" & findstr /r "^OCEANS_X_API_KEY=[^ ]" .env >nul 2>&1 && (set "LOADED_KEYS=!LOADED_KEYS! Oceans-X") || (set "M_OCX=1" & set "MISSING_KEYS=1")

:: Optional Intelligence Audit
set "M_OTX=0" & findstr /r "^OTX_API_KEY=[^ ]" .env >nul 2>&1 || (set "M_OTX=1" & set "MISSING_KEYS=1")
set "M_VTS=0" & findstr /r "^VIRUSTOTAL_API_KEY=[^ ]" .env >nul 2>&1 || (set "M_VTS=1" & set "MISSING_KEYS=1")
set "M_IPD=0" & findstr /r "^ABUSEIPDB_API_KEY=[^ ]" .env >nul 2>&1 || (set "M_IPD=1" & set "MISSING_KEYS=1")
set "M_SHD=0" & findstr /r "^SHODAN_API_KEY=[^ ]" .env >nul 2>&1 || (set "M_SHD=1" & set "MISSING_KEYS=1")
set "M_TLG=0" & findstr /r "^TELEGRAM_API_ID=[^ ]" .env >nul 2>&1 || (set "M_TLG=1" & set "MISSING_KEYS=1")
...
:: Dynamic Intelligence Legend (Only shows if there are missing keys)
if "!MISSING_KEYS!"=="1" (
...
echo.
echo  =========================================================
echo  SUCCESS! CATTO v7.0.0 is running at http://localhost:3002
echo  =========================================================
echo.
