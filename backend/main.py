...
async def lifespan(app: FastAPI):
    ...
    if not _MESH_ONLY:
        ...
        # Staggered startup — fires fetchers in priority order over 4 minutes so
        # the server doesn't spike memory/CPU on all feeds simultaneously.
        # run_staggered_startup() schedules each stage in daemon threads and returns
...
async def update_layers(update: LayerUpdate, request: Request):
    ...
    old_ships = is_any_active(
        "ships_mpa", "ships_military", "ships_cargo", "ships_civilian",
        "ships_passenger", "ships_tracked_yachts", "ships_ais_world",
    )
...
_ASIA_LAT_MIN, _ASIA_LAT_MAX = -10.0, 55.0
_ASIA_LNG_MIN, _ASIA_LNG_MAX =  50.0, 155.0

# Approximate 3000 km bounding box centred on Singapore (1.35°N, 103.82°E)
_SG3K_LAT_MIN, _SG3K_LAT_MAX = -26.0, 29.0
_SG3K_LNG_MIN, _SG3K_LNG_MAX =  76.0, 131.0


def _in_asia(item: dict) -> bool:
    lat = item.get("lat")
    lng = item.get("lng")
    if lat is None or lng is None:
        return True  # no position → keep (live MQTT nodes)
    try:
        return _ASIA_LAT_MIN <= float(lat) <= _ASIA_LAT_MAX and _ASIA_LNG_MIN <= float(lng) <= _ASIA_LNG_MAX
    except (TypeError, ValueError):
        return True


def _in_asia_coords(lat, lng) -> bool:
    """Asia bbox check using separate lat/lng values (for items that use 'lon' instead of 'lng')."""
    try:
        return _ASIA_LAT_MIN <= float(lat) <= _ASIA_LAT_MAX and _ASIA_LNG_MIN <= float(lng) <= _ASIA_LNG_MAX
    except (TypeError, ValueError):
        return True
...
def _in_sg_3000km(lat, lng) -> bool:
    """3000 km bounding box from Singapore — used for power plant regional filtering."""
    try:
        return _SG3K_LAT_MIN <= float(lat) <= _SG3K_LAT_MAX and _SG3K_LNG_MIN <= float(lng) <= _SG3K_LNG_MAX
    except (TypeError, ValueError):
        return True
...
def _filter_sigint_by_layers(items: list, active_layers: dict[str, bool]) -> list:
    ...
    filtered = [item for item in items if str(item.get("source") or "").lower() in allowed_sources]

    # Meshtastic MAP API nodes are global (9K+). Cap to Asia bounds so the
    # payload stays manageable. Live MQTT nodes (no position) always pass through.
    if allow_mesh and not allow_aprs:
        filtered = [item for item in filtered if _in_asia(item)]

    return filtered


_AIS_WORLD_SOURCES = frozenset(
    {"unknown", "other", "cargo", "passenger", "yacht", "tanker", "fishing", "tug", "pleasure", "highspeed"}
)


def _filter_ships_by_layers(ships: list, active_layers: dict) -> list:
    """Filter ships based on active layer toggles.

    - ships_mpa: MPA OCEANS-X Singapore port vessels (source == 'mpa_oceans_x')
    - ships_ais_world: Global AIS stream vessels (source in _AIS_WORLD_SOURCES)
    - ships_military / ships_cargo / ships_civilian / ships_passenger / ships_tracked_yachts:
      applied on top of AIS world when that layer is on.
    When ships_ais_world is OFF, AIS-sourced ships are excluded entirely regardless of category.
    Hard cap: 5000 ships to protect frontend memory.
    """
    mpa_on = active_layers.get("ships_mpa", True)
    ais_world_on = active_layers.get("ships_ais_world", False)

    # Category filters (only relevant when AIS world is on)
    military_on = active_layers.get("ships_military", True)
    cargo_on = active_layers.get("ships_cargo", True)
    civilian_on = active_layers.get("ships_civilian", True)
    passenger_on = active_layers.get("ships_passenger", True)
    yacht_on = active_layers.get("ships_tracked_yachts", True)

    out = []
    for s in ships:
        source = str(s.get("source") or "").lower()

        if source == "mpa_oceans_x":
            if mpa_on:
                out.append(s)
            continue

        # USNI / other tracked non-AIS (military carriers etc.)
        if source not in _AIS_WORLD_SOURCES:
            if military_on:
                out.append(s)
            continue

        # AIS stream vessel — only include when ships_ais_world is on
        if not ais_world_on:
            continue

        if s.get("yacht_alert") and yacht_on:
            out.append(s)
            continue

        vtype = str(s.get("type") or "unknown")
        if (vtype in ("carrier", "military_vessel")) and military_on:
            out.append(s)
        elif (vtype in ("tanker", "cargo")) and cargo_on:
            out.append(s)
        elif vtype == "passenger" and passenger_on:
            out.append(s)
        elif civilian_on:
            out.append(s)
...
async def live_data_fast(
    ...
    # CCTV: hard-cap to prevent payload blowout.
    # SG-only (default): ~90 Singapore LTA cameras.
    # Global (opt-in): capped at 500 to prevent 13K payload crash.
    _cctv_raw = d.get("cctv") or []
    if not active_layers.get("cctv", True):
        _cctv_out = []
    elif active_layers.get("cctv_global", False):
        _cctv_out = _cctv_raw[:500]  # hard cap even when global is on
    else:
        _cctv_out = [c for c in _cctv_raw if c.get("source_agency") == "Singapore LTA"]
    cctv_total = len(_cctv_raw)  # keep total count for UI badge

    # SIGINT: hard-cap at 3000 items to protect renderer memory.
    sigint_items = _filter_sigint_by_layers(d.get("sigint") or [], active_layers)
    if len(sigint_items) > 3000:
        sigint_items = sigint_items[:3000]
    ...
    payload = {
        ...
        "ships": _filter_ships_by_layers(d.get("ships") or [], active_layers) if ships_enabled else [],
        "cctv": _cctv_out,
        ...
        "freshness": freshness,
        "startup_stages": get_startup_registry(),
    }
    return Response(
...
async def live_data_slow(
    ...
    payload = {
        ...
        "kiwisdr": (
            (d.get("kiwisdr") or []) if active_layers.get("kiwisdr_global", False)
            else [k for k in (d.get("kiwisdr") or []) if _in_asia_coords(k.get("lat", 0), k.get("lon", k.get("lng", 0)))]
            if active_layers.get("kiwisdr", False)
            else []
        ),
        ...
        "datacenters": (
            (d.get("datacenters") or []) if active_layers.get("datacenters_global", False)
            else [dc for dc in (d.get("datacenters") or []) if _in_asia_coords(dc.get("lat", 0), dc.get("lng", 0))]
            if active_layers.get("datacenters", True)
            else []
        ),
        "military_bases": (d.get("military_bases") or []) if active_layers.get("military_bases", True) else [],
        "power_plants": (
            (d.get("power_plants") or []) if active_layers.get("power_plants_global", False)
            else [p for p in (d.get("power_plants") or []) if _in_sg_3000km(p.get("lat", 0), p.get("lng", 0))]
            if active_layers.get("power_plants", True)
            else []
        ),
        ...
        "mpa_vessel_types": d.get("mpa_vessel_types") or {},
        "mpa_weather_4day": d.get("mpa_weather_4day") or [],
        "mpa_wind_readings": d.get("mpa_wind_readings") or [],
        "piracy_incidents": (d.get("piracy_incidents") or []) if active_layers.get("piracy_incidents", True) else [],  # global, always visible when on
        "telegram_posts": d.get("telegram_posts") or [],
        "freshness": freshness,
    }
    return Response(
...
def _cctv_upstream_headers(request: Request, profile: _CCTVProxyProfile) -> dict[str, str]:
    headers = {
        "User-Agent": "Mozilla/5.0 (compatible; Catto-OSINT/4.0)",
        **profile.headers,
    }
...
@app.get("/api/mpa/vessel-particulars")
@limiter.limit("30/minute")
async def mpa_vessel_particulars(request: Request, callsign: str = Query(..., min_length=1, max_length=20)):
    """Fetch MPA Oceans-X vessel particulars by callsign (on-demand)."""
    try:
        from services.fetchers.mpa_supplementary import get_vessel_particulars
        data = get_vessel_particulars(callsign)
        if not data:
