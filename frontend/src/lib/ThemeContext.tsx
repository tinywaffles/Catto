...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('catto-theme') as Theme | null;
    if (saved === 'light' || saved === 'dark') {
      setTheme(saved);
      document.documentElement.setAttribute('data-theme', saved);
    }
    const savedHud = localStorage.getItem('catto-hud-color') as HudColor | null;
  ...
  const toggleTheme = () => {
    ...
    setTheme(next);
    localStorage.setItem('catto-theme', next);
  ...
  const cycleHudColor = () => {
    ...
    setHudColor(next);
    localStorage.setItem('catto-hud-color', next);
