...
/** Reverse geocode via BigDataCloud (direct browser call, no key, reliable CORS). */
async function reverseGecodeBigDataCloud(lat: number, lng: number) {
  ...
  return {
    ...
    display_name: [data.locality, data.principalSubdivision, data.countryName].filter(Boolean).join(', '),
  };
}

/** Reverse geocode — tries Nominatim first, falls back to BigDataCloud. */
async function reverseGeocode(lat: number, lng: number) {
  // Try Nominatim first (best place names, but may rate-limit)
  try {
    const url =
  ...
  // Fallback: BigDataCloud (no key, always CORS-enabled)
  return reverseGecodeBigDataCloud(lat, lng);
}
