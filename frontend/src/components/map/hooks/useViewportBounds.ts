...
export function useViewportBounds(
  ...
  const renderBoundsTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const updateBounds = useCallback(() => {
    ...
    // Debounce render bounds to 500ms — prevents excessive GeoJSON rebuilds while panning/zooming
    if (renderBoundsTimerRef.current) clearTimeout(renderBoundsTimerRef.current);
    renderBoundsTimerRef.current = setTimeout(() => {
