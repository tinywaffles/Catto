...
// Format time as SGT HH:mm
function formatTime(pubDate: string) {
    try {
        return new Date(pubDate).toLocaleTimeString(SGT_LOCALE, {
            hour: '2-digit', minute: '2-digit', timeZone: SGT_TZ, hour12: false,
        }) + ' SGT';
    } catch {
        return '';
    }
...
// Relative age: "2h ago", "35m ago", etc.
...
function NewsFeedInner({ selectedEntity, regionDossier, regionDossierLoading, onArticleClick }: { selectedEntity?: SelectedEntity | null, regionDossier?: RegionDossier | null, regionDossierLoading?: boolean, onArticleClick?: (idx: number, lat?: number, lng?: number) => void }) {
    ...
    const [cyberIntelExpanded, setCyberIntelExpanded] = useState(false);
    const [ciiFilterActive, setCiiFilterActive] = useState(true);
    const [regionalNews, setRegionalNews] = useState<{ source: string; title: string; link: string; published: string; description: string }[]>([]);
    ...
    // ICS/SCADA vendors for CII filter (Critical Information Infrastructure)
    const CII_VENDORS = [
        'siemens', 'rockwell', 'allen-bradley', 'schneider', 'honeywell', 'abb',
        'emerson', 'mitsubishi', 'ge', 'yokogawa', 'beckhoff', 'advantech',
        'delta', 'iconics', 'osisoft', 'aveva', 'wonderware', 'inductive',
        'omron', 'fanuc', 'moxa', 'phoenix contact', 'pilz', 'wago', 'codesys',
    ];

    // Normalise all cyber-intel feeds into a single sorted card list.
    // Sort order: 1. Singapore (SingCERT + MRT) → 2. Geopolitical (OTX) → 3. CVEs (CISA KEV) → 4. General (Feodo + Ransomware)
    const cyberCards = useMemo(() => {
        type Sev = 'CRITICAL' | 'HIGH' | 'MEDIUM' | 'LOW';
        type Card = { source: string; timestamp: string; severity: Sev; title: string; description: string; extra: string; link: string; category: number; cii?: boolean; sgSea?: boolean };
        const cards: Card[] = [];

        // ── 1. Singapore threats (SingCERT advisories + MRT disruptions) ──
        for (const e of (data?.singcert_advisories || [])) {
            const ea = e as any;
            cards.push({
                source: 'SingCERT',
                timestamp: ea.date || '',
                severity: (ea.severity || 'HIGH') as Sev,
                title: ea.title || 'Security Advisory',
                description: ea.description || '',
                extra: ea.advisory_id ? `Advisory ID: ${ea.advisory_id}` : '',
                link: ea.url || 'https://www.csa.gov.sg/singcert/advisories',
                category: 1,
            });
        }
        for (const e of (data?.mrt_alerts || [])) {
            const ea = e as any;
            cards.push({
                source: 'LTA MRT',
                timestamp: ea.created_date || '',
                severity: 'HIGH',
                title: 'MRT Service Disruption',
                description: ea.message || '',
                extra: ea.affected?.map((s: any) => s.station).join(', ') || '',
                link: 'https://www.lta.gov.sg/content/ltagov/en/getting_around/public_transport/trains.html',
                category: 1,
            });
        }

        // ── 2. Geopolitical (OTX pulses) ──
        for (const e of (data?.otx_pulses || [])) {
            const tlp = ((e as any).tlp || '').toLowerCase();
            const sev: Sev = tlp === 'red' ? 'CRITICAL' : tlp === 'amber' ? 'HIGH' : 'MEDIUM';
            const tags: string[] = Array.isArray((e as any).tags) ? (e as any).tags : [];
            const isCii = tags.some(t => CII_VENDORS.some(v => t.toLowerCase().includes(v))) ||
                ['ics', 'scada', 'industrial', 'singapore', 'sg-cert'].some(k => tags.some(t => t.toLowerCase().includes(k)));
            if (ciiFilterActive && !isCii) continue;
            cards.push({
                source: 'AlienVault OTX',
                timestamp: (e as any).modified || (e as any).created || '',
                severity: sev,
                title: (e as any).name || 'OTX Pulse',
                description: (e as any).description || tags.slice(0, 5).join(', ') || '',
                extra: `${(e as any).indicator_count ?? 0} IOCs · TLP:${tlp.toUpperCase() || 'WHITE'}`,
                link: (e as any).id ? `https://otx.alienvault.com/pulse/${(e as any).id}` : 'https://otx.alienvault.com/',
                category: 2,
                cii: isCii,
            });
        }

        // ── 3. General (Feodo C2 + Ransomware IOCs) ──
        for (const e of (data?.feodo_c2 || [])) {
            const ip = (e as any).ip_address || '';
            const port = (e as any).port || '';
            const asName = (e as any).as_name || '';
            // Feodo always included — appears below CII-tagged entries (category sort)
            cards.push({
                source: 'Feodo Tracker',
                timestamp: (e as any).first_seen_utc || '',
                severity: (e as any).status === 'online' ? 'CRITICAL' : 'HIGH',
                title: `${(e as any).malware || 'Botnet'} C2`,
                description: `${ip}:${port}${asName ? ` — ${asName}` : ''}`,
                extra: (e as any).country || '',
                link: ip ? `https://feodotracker.abuse.ch/browse.php?search=${ip}` : 'https://feodotracker.abuse.ch/',
                category: 4,
            });
        }
        // ── 4. Ransomware.live victims ──
        const SEA_COUNTRIES = new Set(['SG', 'MY', 'TH', 'ID', 'PH', 'VN', 'MM', 'BN', 'KH', 'LA', 'TL']);
        for (const e of (data?.ransomware_iocs || [])) {
            if (ciiFilterActive) continue;
            const ea = e as any;
            const country = (ea.country || '').toUpperCase();
            const isSgSea = SEA_COUNTRIES.has(country);
            cards.push({
                source: 'Ransomware.live',
                timestamp: ea.discovered || '',
                severity: isSgSea ? 'CRITICAL' : 'HIGH',
                title: ea.victim || 'Unknown Victim',
                description: ea.domain || '',
                extra: `${ea.group}${country ? ` · ${country}` : ''}`,
                link: ea.domain ? `https://www.google.com/search?q=${encodeURIComponent(ea.victim)}+ransomware` : 'https://ransomware.live/',
                category: 4,
                sgSea: isSgSea,
            });
        }
        ...
        // Sort: severity first (CRITICAL→HIGH→MEDIUM→LOW), then timestamp desc within each severity
        ...
        return cards;
    }, [data?.ransomware_iocs, data?.feodo_c2, data?.otx_pulses, data?.singcert_advisories, data?.mrt_alerts, ciiFilterActive, regionalNews]);

    // Regional news — fetch once on mount, refresh every 10 min
    useEffect(() => {
        let cancelled = false;
        let timer: ReturnType<typeof setTimeout>;
        async function load() {
            try {
                const r = await fetch('/api/regional-news');
                if (!cancelled && r.ok) setRegionalNews(await r.json());
            } catch { /* ignore */ }
            if (!cancelled) timer = setTimeout(load, 10 * 60 * 1000);
        }
        void load();
    ...
    // Bus Stop arrivals
    const [busArrivals, setBusArrivals] = useState<{ service_no: string; operator: string; next: { eta_mins: number; load: string; type: string }[] }[]>([]);
    const [busLoading, setBusLoading] = useState(false);
    useEffect(() => {
        if (selectedEntity?.type !== 'bus_stop') { setBusArrivals([]); return; }
        const code = (selectedEntity.extra as any)?.code || selectedEntity.id;
        if (!code) return;
        setBusLoading(true);
        fetch(`/api/sg/bus-arrival?stop=${encodeURIComponent(String(code))}`)
            .then(r => r.ok ? r.json() : { services: [] })
            .then(d => { setBusArrivals(d.services || []); })
            .catch(() => { setBusArrivals([]); })
            .finally(() => setBusLoading(false));
    }, [selectedEntity]);

    if (selectedEntity?.type === 'bus_stop') {
        const extra = (selectedEntity.extra || {}) as any;
        const LOAD_LABEL: Record<string, string> = { SEA: 'SEATS AVL', SDA: 'STANDING', LSD: 'FULL' };
        const LOAD_COLOR: Record<string, string> = { SEA: 'text-green-400', SDA: 'text-yellow-400', LSD: 'text-red-400' };
        return (
            <motion.div
                key="bus-stop-panel"
                ...
                className="w-full bg-[#0a0a0a]/90 backdrop-blur-sm border border-cyan-900/40 flex flex-col z-10 font-mono pointer-events-auto overflow-hidden flex-shrink-0"
            >
                <div className="p-3 border-b border-cyan-800/40 bg-cyan-950/20 flex justify-between items-center">
                    <h2 className="text-xs tracking-widest font-bold text-cyan-400">BUS STOP {extra.code || selectedEntity.id}</h2>
                    <span className="text-[8px] text-[var(--text-muted)] uppercase tracking-widest">{extra.road || extra.desc || ''}</span>
                </div>
                <div className="p-3 flex flex-col gap-2 max-h-96 overflow-y-auto styled-scrollbar">
                    {extra.desc && (
                        <div className="text-[9px] text-[var(--text-secondary)] tracking-widest">{extra.desc}</div>
                    )}
                    {busLoading ? (
                        <div className="text-[9px] text-cyan-400 animate-pulse">FETCHING ARRIVALS...</div>
                    ) : busArrivals.length === 0 ? (
                        <div className="text-[9px] text-[var(--text-muted)]">No services detected at this stop.</div>
                    ) : (
                        <div className="flex flex-col gap-1.5">
                            {busArrivals.map((svc) => (
                                <div key={svc.service_no} className="flex items-start gap-2 border border-cyan-900/30 bg-black/40 px-2 py-1.5 rounded-sm">
                                    <span className="text-[13px] font-bold text-cyan-300 w-10 shrink-0">{svc.service_no}</span>
                                    <div className="flex gap-3 flex-wrap">
                                        {svc.next.map((bus, i) => (
                                            <div key={i} className="flex flex-col items-center gap-0.5">
                                                <span className="text-[11px] font-bold text-white">
                                                    {bus.eta_mins === 0 ? 'ARR' : `${bus.eta_mins}m`}
                                                </span>
                                                <span className={`text-[7px] font-mono ${LOAD_COLOR[bus.load] ?? 'text-cyan-500'}`}>
                                                    {LOAD_LABEL[bus.load] ?? bus.load}
                                                </span>
                                                {bus.type === 'DD' && <span className="text-[6px] text-purple-400 font-mono">2-DECK</span>}
                                            </div>
                                        ))}
                                    </div>
                                </div>
            ...
            return (
                ...
                >
                    ...
                    <div className="p-4 flex flex-col gap-3">
                        ...
                        {(flight.registration || flight.icao24) && (
                            <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
                                <span className="text-[var(--text-muted)] text-[10px]">AIRFRAMES.IO</span>
                                <a
                                    href={flight.registration
                                        ? `https://www.airframes.io/reg/${encodeURIComponent(flight.registration.replace(/\s/g, ''))}`
                                        : `https://www.airframes.io/hex/${flight.icao24}`}
                                    target="_blank"
                                    rel="noreferrer"
                                    className={`${headerColor} hover:opacity-80 text-xs font-bold underline`}
                                >
                                    Lookup Aircraft →
                                </a>
                            </div>
                ...
                >
                    ...
                    <div className="p-4 flex flex-col gap-3">
                        ...
                        <div className="flex justify-between items-center pt-1">
                            <span className="text-[var(--text-muted)] text-[10px]">WATCHLIST</span>
                            <button
                                onClick={() => {
                                    const query = callsign !== 'UNKNOWN' ? callsign : (flight.registration || flight.icao24 || '');
                                    if (!query) return;
                                    addWatchlistEntry(query, {
                                        callsign: callsign !== 'UNKNOWN' ? callsign : undefined,
                                        registration: flight.registration || undefined,
                                        aircraftType: flight.model || undefined,
                                    });
                                    emitToast({
                                        id: `watchlist-add-${query}`,
                                        title: `${query} added to watchlist`,
                                        source: 'WATCHLIST',
                                        severity: 'HIGH',
                                    });
                                }}
                                className="text-amber-400 hover:text-amber-300 text-xs font-bold border border-amber-500/30 hover:border-amber-400/60 px-2 py-0.5 rounded transition-colors"
                            >
                                + Add to Watchlist
                            </button>
                        </div>
                ...
                >
                    ...
                    <div className="p-4 flex flex-col gap-3">
                        ...
                        {item.machine_assessment && (
                            <div className="mt-2 p-2 bg-black/60 border border-cyan-800/50 rounded-sm text-[9px] text-cyan-400 font-mono leading-tight relative overflow-hidden shadow-[inset_0_0_10px_rgba(59,130,246,0.05)]">
    ...
    return (
        ...
        >
                                ...
                                >
                                    ...
                                    <div className="px-2 py-1 border-t border-purple-500/10 flex justify-between items-center">
                                        ...
                                        <span className="text-[7px] text-[var(--text-muted)]">
                                            {fimi.last_fetched ? new Date(fimi.last_fetched).toLocaleString(SGT_LOCALE, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', timeZone: SGT_TZ, hour12: false }) + ' SGT' : ''}
                                        </span>
                                    </div>
                                </motion.div>
                            )}
            ...
            {/* ── CATTO INTEL (unified cyber + regional) ──────────────── */}
            <AnimatePresence>
                {!isMinimized && cyberCards.length > 0 && (
                    <motion.div
                        initial={{ height: 0, opacity: 0 }}
                        animate={{ height: "auto", opacity: 1 }}
                        exit={{ height: 0, opacity: 0 }}
                    ...
                    >
                        <div className="flex items-center gap-1">
                            <button
                                onClick={() => setCyberIntelExpanded(!cyberIntelExpanded)}
                                className="flex-1 flex items-center gap-2 px-2 py-1.5 border rounded-sm font-mono cursor-pointer transition-colors bg-red-950/30 border-red-500/40 hover:bg-red-950/50"
                            >
                                <div className="w-2 h-2 rounded-full bg-red-400 animate-pulse" />
                                <span className="text-[9px] font-bold tracking-wider text-red-400">
                                    ⚡ ASIA NEWS
                                </span>
                                <span className="text-[8px] text-[var(--text-muted)] ml-auto flex items-center gap-1">
                                    <span>{cyberCards.length} ENTRIES</span>
                                    {cyberIntelExpanded ? <ChevronUp size={10} /> : <ChevronDown size={10} />}
                                </span>
                            </button>
                            <button
                                onClick={() => setCiiFilterActive(f => !f)}
                                title="Filter: Singapore ICS/SCADA (CII)"
                                className={`flex-shrink-0 px-1.5 py-1.5 border rounded-sm font-mono text-[7px] font-bold tracking-wider transition-colors ${
                                    ciiFilterActive
                                        ? 'bg-orange-950/60 border-orange-500/60 text-orange-400'
                                        : 'bg-transparent border-red-500/20 text-[var(--text-muted)] hover:border-orange-500/40 hover:text-orange-400'
                                }`}
                            >
                                CII
                            </button>
                        ...
                        <AnimatePresence>
                            {cyberIntelExpanded && (
                                <motion.div
                                    ...
                                    animate={{ height: "auto", opacity: 1 }}
                                    exit={{ height: 0, opacity: 0 }}
                                    className="mt-1 border border-red-500/20 bg-black/40 rounded-sm overflow-hidden"
                                >
                                    <div className="overflow-y-auto max-h-64 styled-scrollbar flex flex-col gap-1 p-2">
                                        {cyberCards.slice(0, 150).map((card, i) => {
                                            const sevColors: Record<string, { bg: string; badge: string }> = {
                                                CRITICAL: { bg: 'bg-red-950/30 border-red-500/50',    badge: 'bg-red-500/20 text-red-300 border-red-400/50' },
                                                HIGH:     { bg: 'bg-orange-950/20 border-orange-500/30', badge: 'bg-orange-500/10 text-orange-400 border-orange-500/30' },
                                                MEDIUM:   { bg: 'bg-yellow-950/20 border-yellow-500/30', badge: 'bg-yellow-500/10 text-yellow-400 border-yellow-500/30' },
                                                LOW:      { bg: 'bg-green-950/20 border-green-500/30',  badge: 'bg-green-500/10 text-green-400 border-green-500/30' },
                                            };
                                            const { bg, badge } = sevColors[card.severity] ?? sevColors.LOW;
                                            const cat = (card as any).category as number;
                                            const catLabel = cat === 1 ? 'SG' : cat === 2 ? 'GEO' : cat === 3 ? 'REG' : 'GEN';
                                            const catColor = cat === 1
                                                ? 'bg-red-950/50 border-red-700/40 text-red-400'
                                                : cat === 3
                                                    ? 'bg-amber-950/50 border-amber-700/40 text-amber-400'
                                                    : 'bg-cyan-950/50 border-cyan-800/40 text-cyan-500';
                                            const isCii = (card as any).cii === true;
                                            const isSgSea = (card as any).sgSea === true;
                                            return (
                                                <div key={i} className={`p-2 rounded-sm border-l-[2px] border-r border-t border-b ${bg} flex flex-col gap-1 shrink-0`}>
                                                    <div className="flex items-center justify-between text-[8px] uppercase tracking-widest">
                                                        <span className="font-bold text-white flex items-center gap-1">
                                                            &gt;_ {card.source}
                                                            <span className={`text-[6px] px-1 py-0.5 border font-mono ${catColor}`}>{catLabel}</span>
                                                            {isCii && <span className="text-[6px] px-1 py-0.5 bg-orange-950/50 border border-orange-700/40 text-orange-400 font-mono">CII</span>}
                                                            {isSgSea && <span className="text-[6px] px-1 py-0.5 bg-red-900/60 border border-red-500/60 text-red-300 font-mono animate-pulse">SG/SEA</span>}
                                                        </span>
                                                        <span className="text-[var(--text-muted)]">{card.timestamp ? relativeTime(card.timestamp) : '—'}</span>
                                                    </div>
                                                    <div className="text-[10px] text-cyan-300 font-bold leading-tight line-clamp-2">
                                                        {card.title}
                                                    </div>
                                                    {card.description && (
                                                        <div className="text-[8.5px] text-[var(--text-muted)] leading-snug line-clamp-2">
                                                            {card.description}
                                                        </div>
                                                    )}
                                                    <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                                                        <span className={`text-[8px] font-bold font-mono px-1.5 py-0.5 rounded-sm border ${badge}`}>
                                                            {card.severity}
                                                        </span>
                                                        {card.extra && (
                                                            <span className="text-[7px] text-[var(--text-muted)] font-mono truncate max-w-[160px]">
                                                                {card.extra}
                                                            </span>
                                                        )}
                                                        <a
                                                            ...
                                                            target="_blank"
                                                            rel="noreferrer"
                                                            className="text-[7px] text-cyan-500 hover:text-cyan-300 transition-colors ml-auto"
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center justify-between text-[8px] text-[var(--text-secondary)] uppercase tracking-widest">
                                        ...
                                        <span className="text-[var(--text-muted)]">{item.published ? relativeTime(item.published) : ''}</span>
                                    </div>
                                    ...
                                    {item.machine_assessment && (
                                        <div className="mt-1 p-1.5 bg-black/60 border border-cyan-800/50 rounded-sm text-[8.5px] text-cyan-400 font-mono leading-tight relative overflow-hidden shadow-[inset_0_0_10px_rgba(59,130,246,0.05)]">
                                            <div className="absolute top-0 left-0 w-[2px] h-full bg-cyan-500 animate-pulse"></div>
                                            <span className="font-bold text-white">&gt;_ SYS.ANALYSIS: </span>
                                            <span className="text-cyan-300 opacity-90">{item.machine_assessment}</span>
                                        </div>
